...
FINGERPRINTS: dict[str, tuple[str, bool]] = {
    ...
    '026_partial_uniq_pos': ("index:CREATE UNIQUE INDEX uq_cm_contract_requests_boond_positioning ON public.cm_contract_requests USING btree (boond_positioning_id) WHERE ((status)::text <> 'cancelled'::text)", True),
    ...
    '031_remove_siren_unique': ('index:CREATE INDEX ix_tp_third_parties_siren ON public.tp_third_parties USING btree (siren)', True),
