...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "charter_template_id",
            UUID(as_uuid=True),
            sa.ForeignKey("cm_charter_templates.id"),
            nullable=False,
        ),
        sa.Column(
            "third_party_id",
        ...
        sa.Column(
            ...
            sa.String(255),
            nullable=True,
            comment="Null for partner acknowledgements",
        ),
        sa.Column("consultant_email", sa.String(255), nullable=True),
        sa.Column(
            "acknowledged_at", sa.DateTime(), nullable=False, server_default=sa.text("now()")
        ),
