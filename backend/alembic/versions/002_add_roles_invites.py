...
def upgrade() -> None:
    ...
    op.add_column(
        "opportunities", sa.Column("owner_id", postgresql.UUID(as_uuid=True), nullable=True)
    )

    op.create_index(
        "ix_opportunities_manager_boond_id", "opportunities", ["manager_boond_id"], unique=False
    )
    ...
    op.create_index(
        "ix_business_leads_submitter_id", "business_leads", ["submitter_id"], unique=False
    )
