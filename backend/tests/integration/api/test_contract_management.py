...
async def _create_contract_request(db: AsyncSession, **overrides) -> ContractRequestModel:
    ...
    defaults = {
        "id": uuid4(),
        "provisional_reference": f"PROV-2026-{uuid4().int % 9999:04d}",
...
class TestCandidateStateWebhook:
    """Tests for POST /api/v1/webhooks/boondmanager/candidate-state-update."""

    @pytest.mark.asyncio
    async def test_always_returns_200(self, client: AsyncClient):
        """Webhook always returns 200 OK (even on error)."""
        response = await client.post(
            "/api/v1/webhooks/boondmanager/candidate-state-update",
            json={"some": "data"},
        )
    ...
    @pytest.mark.asyncio
    async def test_filters_wrong_state(self, client: AsyncClient):
        """Webhook filters out non-state-11 events."""
        payload = [
            {
                "data": {
                    "id": "3_test",
                    "type": "webhookevent",
                    "relationships": {
                        "dependsOn": {"id": "42", "type": "candidate"},
                    },
                    "included": [
                        {
                            "type": "log",
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["message"] == "No action taken"


class TestResourceStateWebhook:
    """Tests for POST /api/v1/webhooks/boondmanager/resource-state-update."""

    @pytest.mark.asyncio
    async def test_always_returns_200(self, client: AsyncClient):
        """Webhook always returns 200 OK (even on error)."""
        response = await client.post(
            "/api/v1/webhooks/boondmanager/resource-state-update",
            json={"some": "data"},
        )
    ...
    @pytest.mark.asyncio
    async def test_filters_wrong_state(self, client: AsyncClient):
        """Webhook filters out non-state-4/5 events."""
        payload = [
            {
                "data": {
                    "id": "3_test",
