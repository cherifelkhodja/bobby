...
class TestComplianceChecker:
    ...
    def test_no_docs_is_non_compliant(self):
        """Given no documents, compliance is NON_COMPLIANT (company info not submitted)."""
        result = compute_compliance_status([])
        assert result == ComplianceStatus.NON_COMPLIANT

    def test_ei_all_validated_is_compliant(self):
        """Given an EI with all required docs validated, compliance is COMPLIANT."""
        requirements = REQUIREMENTS_BY_ENTITY_CATEGORY["ei"]
        docs = [
            self._make_doc(req["type"], DocumentStatus.VALIDATED)
            for req in requirements
            if req["mandatory"]
        ]
        result = compute_compliance_status(docs)
        assert result == ComplianceStatus.COMPLIANT

    def test_societe_all_validated_is_compliant(self):
        """Given a société with all required docs validated, compliance is COMPLIANT."""
        requirements = REQUIREMENTS_BY_ENTITY_CATEGORY["societe"]
        docs = [
            self._make_doc(req["type"], DocumentStatus.VALIDATED)
            for req in requirements
            if req["mandatory"]
    ...
    def test_one_missing_gives_non_compliant(self):
        """Given one mandatory doc missing, compliance is NON_COMPLIANT."""
        requirements = REQUIREMENTS_BY_ENTITY_CATEGORY["societe"]
        mandatory = [r for r in requirements if r["mandatory"]]
