...
- **Objets d'emails fournisseur explicites** : les emails destinés au sous-traitant précisent la société émettrice et le contexte — `[Société] Sous-traitance {tiers} - Documents requis pour votre référencement`, `[Société] Contrat de sous-traitance {réf} - Votre relecture est attendue` (la référence du contrat transite désormais via `GenerateMagicLinkCommand.contract_ref`), `[Société] Document refusé : {type}`, `[Société] Rappel : documents de sous-traitance en attente (Ne relance)`.
