...
- **Contacts repris plutôt que doublés** : une société déjà dans le CRM a souvent ses contacts. Avant d'en créer un, Bobby lit les contacts de la société (`GET /companies/{id}/contacts`, page par page) et retient celui dont `email1` est l'adresse cherchée ; un homonyme chez un autre client n'est jamais rattaché puisque seule cette société est lue. La recherche est un bonus : en panne, le contact est créé. Une société créée à l'instant n'est pas fouillée. Partagé par l'action manuelle et la synchronisation (`application/boond_supplier.py`).
