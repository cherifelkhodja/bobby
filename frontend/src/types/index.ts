...
export const POR_STATUS_CONFIG: Record<PurchaseOrderRequestStatus, { label: string; color: string; group: 'active' | 'done' | 'blocked' }> = {
  ...
  compliance_expired: { label: 'Documents expirés', color: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300', group: 'blocked' },
  active: { label: 'En cours', color: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300', group: 'active' },
  archived: { label: 'Archivé', color: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300', group: 'done' },
  cancelled: { label: 'Annulé', color: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400', group: 'done' },
};
...
export interface PurchaseOrderRequestItem {
  ...
  original_contract_request_id: string | null;
  status_history: Array<{ status: PurchaseOrderRequestStatus; entered_at: string }>;
  created_at: string;
  updated_at: string;
}

export interface PurchaseOrderRequestListResponse {
